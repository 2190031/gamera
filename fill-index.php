...
if ($result->num_rows > 0) {
    while ($rowP = $result->fetch_assoc()) {
        ...
        '>".$rowP['title']."</a>
        <button type='button' href='#title' class='btn btn-primary btn-sm float-end' id='scroll-nav-btn' onclick='editCont(`".$rowP['id']."`, `".$rowP['title']."`, 1, `". $rowP['content']."`)'>Editar</button>
        <br>";
        ...
        $resultSec = $conn->query($querySec);
            if ($resultSec->num_rows > 0) {
                while ($rowS = $resultSec->fetch_assoc()) {
                        ...
                        '>".$rowS['title']."</a>
                        <button type='button' href='#title' class='btn btn-primary btn-sm float-end' id='scroll-nav-btn' onclick='editCont(`".$rowS['id']."`, `".$rowS['title']."`, 2, `".$rowS['content']."`)'>Editar</button>
                        <br>";
                        if ($resultSec->num_rows > 0) {
                            while ($rowS = $resultSec->fetch_assoc()) {
                                echo "<input id='$rowS[id]' type='radio' value='".$rowS['content']."' name='helpDoc' onchange='showTitleAndCont(`".$rowS['title']."`, this.value)' hidden>
                                ...
                                document.getElementById(`$rowS[id]`).dispatchEvent(event);
                                '>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".$rowS['title']."</a>
                                <button type='button' href='#title' class='btn btn-success btn-sm float-end' id='scroll-nav-btn' onclick='editCont(`".$rowS['id']."`, `".$rowS['title']."`, `". $rowS['content']."`)'>Editar</button>
                                <br>";
                                ...
                                if ($resultTer->num_rows > 0) {
                                    while ($rowT = $resultTer->fetch_assoc()) {
                                        ...
                                        <button type='button' class='btn btn-secondary btn-sm float-end' id='scroll-nav-btn' onclick='editCont(".$rowT['id'].", ".$rowT['title'].", 3, ". $rowT['content']."`)'>Editar</button>
                                        <br>";
                                    }
                                }
                            }
